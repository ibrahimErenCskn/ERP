    ...
    database.ref("Giris").once('value').then(function(snapshot) {
        ...
        database.ref(name+"/stokYonetimi").once('value').then(function(snapshot) {
            ...
            for (let i = 0; i < removeBilgi.length; i++) {
                if (removeBilgi[i]["urunAdi"]==Sira) {
                    removeDataRef = database.ref(name+"/stokYonetimi/"+Object.keys(removedata)[i])
                        console.log(name+"/stokYonetimi/"+Object.keys(removedata)[i])
